...
const getBacklinks = (keyword) => {
  ...
  if (kw.includes('tech') || kw.includes('software') || kw.includes('app')) {
    return [
      ...
      { domain: 'techradar.com', da: 75, email: 'editor@techradar.com', link_type: 'Guest Post', opportunity: 'High', reason: 'Tech authority' },
...
const getKeywords = (keyword) => {
  ...
  if (kw.includes('laptop') || kw.includes('computer')) {
    return [
  ...
  if (kw.includes('shoe') || kw.includes('sneaker')) {
    return [
  ...
  if (kw.includes('cargo') || kw.includes('logistics')) {
    return [
  ...
  if (kw.includes('ai') || kw.includes('chatgpt')) {
    return [
...
export async function POST(request) {
  try {
    ...
    const route = pathname.split('/api/v13/')[1] || '';
