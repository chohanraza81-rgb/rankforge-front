          ...
          <div className="space-y-3">
            ...
            {activeTab === 'keyword' && (
              <input
                ...
                placeholder='Enter keyword (e.g., "best laptops")'
            ...
            {activeTab === 'onpage' && (
              <textarea
                ...
                rows="4"
                className="w-full p-3 rounded-xl bg-white/10 border border-white/20 focus:ring-2 focus:ring-purple-500 outline-none text-white resize-none"
              />
            )}
            ...
            {(activeTab === 'outline' || activeTab === 'brief') && (
              <div className="grid grid-cols-2 gap-3">
                <input
                  type="text"
                  value={keyword}
            ...
            {(activeTab === 'backlink' || activeTab === 'trend' || activeTab === 'plan') && (
              <input
                type="text"
                value={keyword}
                onChange={(e) => setKeyword(e.target.value)}
            ...
            <button
              ...
              disabled={loading}
              className="w-full py-3.5 bg-gradient-to-r from-purple-600 to-pink-600 hover:scale-[1.02] transition rounded-xl font-bold disabled:opacity-50 flex items-center justify-center gap-2"
            >
          ...
          <div className="bg-white/5 rounded-2xl border border-white/10 p-5">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-semibold text-green-400 flex items-center gap-2">
                ...
                <span className="text-xs bg-green-500/20 text-green-300 px-2 py-0.5 rounded-full">V11 REAL</span>
              </h2>
              <button
                onClick={() => { navigator.clipboard.writeText(JSON.stringify(results, null, 2)); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
                className="px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded-xl text-xs"
              >
                {copied ? '✅ Copied!' : '📋 Copy'}
              </button>
            </div>
            ...
            {activeTab === 'keyword' && results.keywords && (
              <div className="space-y-3 max-h-96 overflow-y-auto">
                <div className="text-sm text-gray-400">{results.keywords.length} real keywords</div>
                  ...
                  <div key={i} className="p-3 rounded-xl border border-white/10 bg-white/5">
                    <div className="flex flex-wrap justify-between items-center gap-2">
                      ...
                      <div className="flex flex-wrap gap-1.5 text-[10px]">
                        ...
                        <span className="px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-300">{kw.intent}</span>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            {activeTab === 'competitor' && results.competitors && (
              <div className="space-y-3 max-h-96 overflow-y-auto">
                <div className="text-sm text-gray-400">{results.competitors.length} real competitors</div>
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        {comp.missing_headings?.length > 0 && (
                          <div className="text-[10px] text-yellow-300 mt-1">📌 {comp.missing_headings.join(', ')}</div>
                        )}
                      </div>
            ...
            {activeTab === 'outline' && results.outline && (
              <div className="space-y-3 max-h-96 overflow-y-auto">
                <div className="bg-purple-500/10 p-3 rounded-xl border border-purple-500/20">
                  ...
                  <div className="text-xs text-gray-400 mt-1">📌 {results.outline.meta_title}</div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                ...
                {results.outline.local_angle && (
                  <div className="bg-green-500/10 p-2 rounded-xl border border-green-500/20 text-xs">
                    🌍 {results.outline.local_angle}
                  </div>
                )}
              </div>
            ...
            {activeTab === 'backlink' && results.backlinks && (
              <div className="space-y-2 max-h-96 overflow-y-auto">
                <div className="text-sm text-gray-400">{results.backlinks.length} real backlink opportunities</div>
            ...
            {activeTab === 'trend' && results.trend && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="bg-white/5 p-3 rounded-xl text-center">
                    <div className="text-sm text-cyan-300">{results.peak_value}</div>
                    <div className="text-[10px] text-gray-400">Peak Value</div>
                  </div>
                  <div className="bg-white/5 p-3 rounded-xl text-center">
                    <div className="text-sm text-green-300">{results.best_publish_date}</div>
